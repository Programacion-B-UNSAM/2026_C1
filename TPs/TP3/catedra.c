...
static void capture_call(void (*fn)(GraphNode*, int), GraphNode* root, int arg, char* buf, int buf_size) {
    ...
    int n = read(pipefd[0], buf, buf_size - 1);
    close(pipefd[0]);
    buf[n < 0 ? 0 : n] = '\0';
...
void testEjercicio1(void) {
    ...
    if (found != 1) {
        printf("Error ej1 caso 1a: findNode(11) = %d (esperado 1)\n", found);
        return;
    }

    found = findNode(root, 9);
    if (found != 1) {
        printf("Error ej1 caso 1b: findNode(9) = %d (esperado 1)\n", found);
        return;
    }

    found = findNode(root, 7);
    if (found != 0) {
        printf("Error ej1 caso 1c: findNode(7) = %d (esperado 0)\n", found);
        return;
    }
    ...
    if (found != 1) {
        printf("Error ej1 caso 2: findNode(1) = %d (esperado 1)\n", found);
        return;
    }
    ...
    if (found != 1) {
        printf("Error ej1 caso 3a: findNode(42) = %d (esperado 1)\n", found);
        return;
    }

    found = findNode(root, 0);
    if (found != 0) {
        printf("Error ej1 caso 3b: findNode(0) = %d (esperado 0)\n", found);
        return;
    }
    ...
    if (found != 1) {
        printf("Error ej1 caso 4: findNode(5) = %d (esperado 1)\n", found);
        return;
    }
    ...
    found = findNode(root, 1);
    if (found != 1) {
        printf("Error ej1 caso 5: findNode(1) = %d (esperado 1)\n", found);
        return;
    }
...
void testEjercicio2(void) {
     ...
     */
    tnode_info_t g1[] = {
        {'\0', 'A', 0}, {'A', 'B', 4}, {'A', 'C', 2}, {'B', 'D', 5}, {'B', 'E', 1}, {'C', 'F', 8},
    };
    createGraph(g1, 6, &root);

    if (!root || root->nombre != 'A') {
        printf("Error ej2 caso 1: raiz incorrecta\n");
        return;
    }
    if (root->num_children != 2) {
        printf("Error ej2 caso 1: A deberia tener 2 hijos, tiene %d\n", root->num_children);
        return;
    }
    ...
    if (!root || root->nombre != 'Z') {
        printf("Error ej2 caso 2: raiz incorrecta\n");
        return;
    }
     ...
     */
    tnode_info_t g5[] = {
        {'\0', 'R', 0}, {'R', 'X', 10}, {'R', 'Y', 1}, {'X', 'P', 1}, {'X', 'Q', 1}, {'Y', 'S', 1},
    };
